import { DEFAULT_SCHOOL_REGION, isSchoolRegionEligible } from './admission-regional-eligibility.mjs';
 ...
 */
export function classifyAdmissionEligibility({
  admissionName = '',
  ...
  if (ELIGIBILITY_VALUES.has(eligibilityType)) {
    return {
      ...
      studentDefaultVisible: eligibilityType === 'general' || eligibilityType === 'school-recommendation' || (eligibilityType === 'regional' && regionalVisible),
    };
  }
  ...
  if (/지역인재|지역혁신인재|지역교과/.test(name)) return { eligibilityType: 'regional', eligibilityVerification: regionalVisible ? 'official-confirmed' : 'name-classified', studentDefaultVisible: regionalVisible };
