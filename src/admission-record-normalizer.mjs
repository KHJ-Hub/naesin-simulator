...
function classifyAcademicFieldFromNormalizedName(name) {
  ...
  if (/(인문|경상|경영|사회과학|법과)대학.*(자유|자율)|인문계열|인문분야|경상계열|인문과학계열|인문대학|경상학부|상경학부|법과대학/.test(name)) return ADMISSION_ACADEMIC_FIELDS.HUMANITIES;
  if (/(공과|자연과학|과학기술|의과|첨단기술.*)대학.*(자유|자율)|자연계열|자연과학계열|공과대학|과학기술대학|자연과학대학|자연과학부|이학융합학부/.test(name)) return ADMISSION_ACADEMIC_FIELDS.NATURAL;
  if (/(예술|체육)대학.*(자유|자율)|예체능분야|예체능전공|문화산업대학\(예체능\)/.test(name)) return ADMISSION_ACADEMIC_FIELDS.ARTS;
