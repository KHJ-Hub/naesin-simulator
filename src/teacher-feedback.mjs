...
function feedbackCard(item) {
  ...
  return `<article class="teacher-feedback-card${item.status === '신규' ? ' is-new' : ''}" data-feedback-id="${id}"><header><div><span class="teacher-feedback-category">${escapeHtml(item.category)}</span><time datetime="${escapeHtml(item.submittedAt)}">${escapeHtml(formatDate(item.submittedAt))}</time></div><select class="input" data-feedback-status aria-label="처리 상태">${statusOptions(item.status)}</select></header><p class="teacher-feedback-message">${escapeHtml(item.message)}</p><dl class="teacher-feedback-meta"><div><dt>학년</dt><dd>${grade}</dd></div><div><dt>앱 버전</dt><dd>${escapeHtml(item.appVersion || '-')}</dd></div><div><dt>현재 화면</dt><dd>${escapeHtml(item.currentSection || '-')}</dd></div></dl><details class="teacher-feedback-tech"><summary>기술 정보</summary><dl><div><dt>빌드일</dt><dd>${escapeHtml(item.buildDate || '-')}</dd></div><div><dt>화면 크기</dt><dd>${escapeHtml(item.viewport || '-')}</dd></div><div><dt>브라우저</dt><dd>${escapeHtml(item.userAgent || '-')}</dd></div><div><dt>의견 ID</dt><dd>${id}</dd></div></dl></details><label class="teacher-feedback-note">관리자 메모<textarea class="input" rows="2" maxlength="800" data-feedback-note placeholder="처리 내용이나 확인할 사항">${escapeHtml(item.adminNote || '')}</textarea></label><div class="teacher-feedback-card-actions"><button type="button" class="quiet-button" data-feedback-save-note>메모 저장</button></div></article>`;
}
