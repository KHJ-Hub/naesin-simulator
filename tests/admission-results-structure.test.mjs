...
test('대전 공식 2026 모집단위별 결과를 연결한다', () => {
  const rows = admissionResultsByRegion2026.daejeon;
  assert.equal(rows.length, 1_117);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부교과').length, 801);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부종합').length, 316);
...
test('충북 공식 2026 모집단위별 결과를 연결한다', () => {
  const rows = admissionResultsByRegion2026.chungbuk;
  assert.equal(rows.length, 1_097);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부교과').length, 840);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부종합').length, 257);
  assert.ok(rows.every((item) => item.dataAvailability === 'confirmed-cut'));
});

test('광주 공식 2026 모집단위별 결과를 연결한다', () => {
  const rows = admissionResultsByRegion2026.gwangju;
  assert.equal(rows.length, 668);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부교과').length, 484);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부종합').length, 184);
  assert.ok(rows.every((item) => item.dataAvailability === 'confirmed-cut'));
});

test('전남 공식 2026 모집단위별 결과를 캠퍼스별로 연결한다', () => {
  const rows = admissionResultsByRegion2026.jeonnam;
  assert.equal(rows.length, 319);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부교과').length, 256);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부종합').length, 63);
  assert.equal(rows.filter((item) => item.university === '전남대학교(제2캠퍼스)').length, 31);
});

test('전북 공식 2026 모집단위별 결과를 연결한다', () => {
  const rows = admissionResultsByRegion2026.jeonbuk;
  assert.equal(rows.length, 589);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부교과').length, 385);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부종합').length, 204);
  assert.ok(rows.every((item) => item.dataAvailability === 'confirmed-cut'));
});

test('강원 공식 2026 모집단위별 결과를 연결한다', () => {
  const rows = admissionResultsByRegion2026.gangwon;
  assert.equal(rows.length, 916);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부교과').length, 586);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부종합').length, 330);
  assert.ok(rows.every((item) => item.dataAvailability === 'confirmed-cut'));
});

test('제주 공식 2026 모집단위별 결과를 연결한다', () => {
  const rows = admissionResultsByRegion2026.jeju;
  assert.equal(rows.length, 119);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부교과').length, 101);
  assert.equal(rows.filter((item) => item.admissionCategory === '학생부종합').length, 18);
  assert.ok(rows.every((item) => item.university === '제주대학교'));
});
