import test from 'node:test';
import assert from 'node:assert/strict';
import { UNIVERSITY_AUDIT_2026, UNIVERSITY_AUDIT_SUMMARY_2026 } from '../src/data/university-audit-2026.mjs';

test('서울·경기·인천 대학은 결과 레코드와 별도로 모두 감사 대상이다', () => {
  assert.ok(UNIVERSITY_AUDIT_2026.length >= 85);
  assert.ok(UNIVERSITY_AUDIT_2026.every((item) => item.referenceYear === 2026 && item.universityId && item.subjectAdmissionStatus && item.comprehensiveAdmissionStatus));
});

test('경기 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.경기도.total, 36);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.경기도.audited, 36);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.경기도.unconfirmed, []);
});

test('인천 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.인천광역시.total, 6);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.인천광역시.audited, 6);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.인천광역시.unconfirmed, []);
});

test('부산 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.부산광역시.total, 14);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.부산광역시.audited, 14);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.부산광역시.unconfirmed, []);
});

test('울산 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.울산광역시.total, 1);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.울산광역시.audited, 1);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.울산광역시.unconfirmed, []);
});

test('경남 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.경상남도.total, 11);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.경상남도.audited, 11);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.경상남도.unconfirmed, []);
});

test('대구 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.대구광역시.total, 3);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.대구광역시.audited, 3);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.대구광역시.unconfirmed, []);
});

test('경북 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.경상북도.total, 18);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.경상북도.audited, 18);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.경상북도.unconfirmed, []);
});

test('대전 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.대전광역시.total, 11);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.대전광역시.audited, 11);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.대전광역시.unconfirmed, []);
});

test('세종 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.세종특별자치시.total, 3);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.세종특별자치시.audited, 3);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.세종특별자치시.unconfirmed, []);
});

test('충남 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.충청남도.total, 16);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.충청남도.audited, 16);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.충청남도.unconfirmed, []);
});

test('충북 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.충청북도.total, 12);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.충청북도.audited, 12);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.충청북도.unconfirmed, []);
});

test('광주 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.광주광역시.total, 10);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.광주광역시.audited, 10);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.광주광역시.unconfirmed, []);
});

test('전남 공식 목록 전체와 모집단위별 상세 결과가 감사 상태로 정리된다', async () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.전라남도.total, 10);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.전라남도.audited, 10);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.전라남도.unconfirmed, []);
  const { admissionResults2026Jeonnam } = await import('../src/admission-results/2026/jeonnam.mjs');
  assert.equal(admissionResults2026Jeonnam.length, 319);
});

test('전북 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.전북특별자치도.total, 10);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.전북특별자치도.audited, 10);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.전북특별자치도.unconfirmed, []);
});

test('강원 공식 목록 전체가 감사 상태로 정리된다', () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.강원특별자치도.total, 14);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.강원특별자치도.audited, 14);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.강원특별자치도.unconfirmed, []);
});

test('제주 공식 목록 전체가 감사 상태로 정리된다', async () => {
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.제주특별자치도.total, 2);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.제주특별자치도.audited, 2);
  assert.deepEqual(UNIVERSITY_AUDIT_SUMMARY_2026.제주특별자치도.unconfirmed, []);
  const { admissionResults2026Jeju } = await import('../src/admission-results/2026/jeju.mjs');
  assert.equal(admissionResults2026Jeju.length, 119);
});

test('서울·경기 공식 감사 결과를 함께 연결한다', () => {
  const gachon = UNIVERSITY_AUDIT_2026.find((item) => item.universityName === '가천대학교');
  const konkuk = UNIVERSITY_AUDIT_2026.find((item) => item.universityName === '건국대학교');
  const seoul = UNIVERSITY_AUDIT_2026.find((item) => item.universityName === '서울대학교');
  assert.equal(gachon.subjectAdmissionStatus, 'not-published');
  assert.equal(gachon.comprehensiveAdmissionStatus, 'confirmed-cut');
  assert.equal(konkuk.subjectAdmissionStatus, 'confirmed-cut');
  assert.equal(konkuk.comprehensiveAdmissionStatus, 'confirmed-cut');
  assert.ok(konkuk.sourceUrls.some((url) => url.includes('adiga.kr')));
  assert.equal(seoul.subjectAdmissionStatus, 'no-subject-admission');
  assert.equal(seoul.comprehensiveAdmissionStatus, 'confirmed-cut');
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.서울특별시.total, 43);
  assert.equal(UNIVERSITY_AUDIT_SUMMARY_2026.서울특별시.audited, 43);
});
