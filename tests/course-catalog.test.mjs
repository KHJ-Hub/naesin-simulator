...
test('일반 융합 선택은 A~E 성취도만, 보건은 P 이수 처리한다', () => {
  ...
  assert.equal(health.gradingType, 'achievement');
...
test('카탈로그에 등급 처리 방식의 명시적 타입이 있다', () => {
  ...
  assert.equal(['grade', 'achievement', 'both'].includes(grade.gradingType), true);
});
test('grade·achievement·both 렌더링 입력 계약을 구분한다', () => {
  assert.deepEqual(gradingInputs('grade'), { grade: true, achievement: false });
  assert.deepEqual(gradingInputs('achievement'), { grade: false, achievement: true });
  assert.deepEqual(gradingInputs('both'), { grade: true, achievement: true });
});
