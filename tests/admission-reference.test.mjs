...
test('공식 확인된 2026 지역 자료만 연결한다', () => {
  ...
  assert.ok(new Set(ADMISSION_REFERENCE_DATA.map((item) => item.region)).has('서울특별시'));
  assert.ok(new Set(ADMISSION_REFERENCE_DATA.map((item) => item.university)).has('건국대학교'));
  assert.ok(ADMISSION_REFERENCE_DATA.length > 1_000);
  assert.ok(ADMISSION_REFERENCE_DATA.every((item) => item.cut70Original === item.cut70 && item.conversionDataset === 'busan-grade5-g2-1sem-15978'));
  assert.ok(ADMISSION_REFERENCE_DATA.every((item) => item.cut50Original == null || Number.isFinite(item.cut50Converted)));
  assert.ok(ADMISSION_REFERENCE_DATA.every((item) => item.cut70Original == null || Number.isFinite(item.cut70Converted)));
